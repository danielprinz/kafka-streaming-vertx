...
public class Entrypoint extends AbstractVerticle {
  ...
  private static final String OUTPUT_TOPIC_EVEN = "even-numbers-output";
  private static final String OUTPUT_TOPIC_ODD = "odd-numbers-output";
  ...
  private void onBatch(final KafkaConsumerRecords<Void, JsonObject> records) {
    ...
    CompositeFuture.all(batchProduced).setHandler(ar -> {
      if (ar.failed()){
  ...
  private HashMap<String, String> consumerOptions() {
    ...
    options.put(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, BOOTSTRAP_SERVERS);
  ...
  private static HashMap<String, String> producerOptions() {
    final HashMap<String, String> options = new HashMap<>();
